...
if !exists("g:rf")
	let g:rf = g:rs.g:re    "region stop
endif

if !exists("g:user_defined_snippets")
	let g:user_defined_snippets = "$HOME/.vim/plugin/c_my_snippets.vim"
endif
		...
		if has_key(i,'kind') && has_key(i,'name') && has_key(i,'signature')
			if (i.kind=='p' || i.kind=='f') && i.name==a:fun  " p is declare, f is definition
				if match(i.signature,'(\s*void\s*)')<0 && match(i.signature,'(\s*)')<0
					...
					let tmp=substitute(tmp,'(\(.*\))',g:rs.'\1'.g:re.')'.g:rs.g:re,'g')
				else
					let tmp=''
				endif
				if (tmp != '') && (index(signature_word,tmp) == -1)
...
let g:template['c']['ff'] = "#ifndef \<c-r>=GetFileName()\<cr>\<CR>#define \<c-r>=GetFileName()\<cr>".
			\"\<cr>\<cr>".g:rs.g:re."\<cr>\<cr>"."#endif  /*\<c-r>=GetFileName()\<cr>*/".repeat("\<up>",3)
...
let g:template['c']['switch'] = "switch (".g:rs."...".g:re.") {\<cr>case ".g:rs."...".g:re." :\<cr>".g:rf."\<cr>\<bs>break;\<cr>case ".
			\g:rs."...".g:re." :\<cr>".g:rf."\<cr>\<bs>break;\<cr>default :\<cr>".g:rf."\<cr>\<bs>break;\<cr>}"
